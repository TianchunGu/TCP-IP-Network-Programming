...
- 服务器端展示了**“生产者-消费者”模型**的雏形：多个服务进程（生产者）将数据放入管道，一个记录进程（消费者）从管道取出数据存盘。
- 客户端展示了**“全双工 I/O”**：利用多进程让发送和接收互不干扰。

#### 服务器端

服务器端：echo_storeserv.c（保存消息的回声服务器）这个服务器在原有的“多进程回声”功能基础上，增加了一个 **“将所有客户端发来的消息保存到文件”** 的功能。

核心逻辑设计： 为了避免多个子进程同时写同一个文件造成冲突（竞态条件），程序设计了一个专门负责写文件的进程。

- 工人进程（Client Handlers）：负责和客户端聊天（回声），收到消息后，顺手把消息扔进管道（Pipe）。
- 记录员进程（File Writer）：专门守在管道的出口，读出消息并写入文件。

#### 客户端

这个客户端其实沿用了第 10 章的代码。它的核心思想是 I/O 分割，即把“读”和“写”分开处理，互不等待。
