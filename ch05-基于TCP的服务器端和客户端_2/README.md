...
核心内容是解决TCP“无数据边界”带来的编程问题，并深入讲解TCP的内部工作原理。
...
> TCP中有滑动窗口协议，所以TCP不会因为缓冲溢出而丢失数据。
...
TCP在实际建立连接过程中，会经过三次对话过程，又称为“三次握手”（Three-Way Handshake）。三次握手过程如下：

![TCP](./img/TCP建立连接.jpg "三次握手过程")
...
![TCP](./img/TCP数据交换过程.jpg "数据交换过程")

TCP套接字数据传输中发生错误:

![TCP](./img/TCP数据传输错误.jpg "数据传输错误")

### *TCP内部工作原理3：断开与套接字的连接*

断开连接时需要双方协商，四次握手 (Four-way handshaking)。

![TCP](./img/TCP断开连接.jpg "断开连接过程")
